...
async function addItem(request, response) {
  const { name, descp, quantity, category_id } = request.body;
  const { rows } = await dbPool.query(
    "INSERT INTO item (name, description, quantity, category_id VLAUES ('$1', '$2', $3, $4) ",
    [name, descp, quantity, category_id],
  );
  response.json(rows);
}
...
async function updateItem(request, response) {
  ...
  const { rows } = await dbPool.query(
    "SET name = '$1', description = '$2' , quantity = '$3', category_id = '$4' WHERE id = '$5'",
...
async function updateItemQuantity(request, response) {
  const { updatedQuantity, id } = request.body;
  const { rows } = await dbPool.query(
    "SET quantity = quantity + $1 WHERE id = $2",
    [updatedQuantity, id],
  );
  response.json(rows);
}
